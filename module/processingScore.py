...
def transformChartData(allScore):
    ...
    for semester in range(0, s_len):
        newScore[0]["TotalGrade"] += round(float(allScore[semester]["figure"]["acq_creidt"]),2)
        newScore[0]["AvgScore"] += round(float(allScore[semester]["figure"]["r_scores_avg"]),2)
        newScore[0]["AvgGradePoints"] += round(float(allScore[semester]["figure"]["rating_avg"]),2)

    newScore[0]["TotalGrade"] = round(newScore[0]["TotalGrade"] / s_len, 2)
    newScore[0]["AvgScore"] = round(newScore[0]["AvgScore"] / s_len, 2)
    newScore[0]["AvgGradePoints"] = round(newScore[0]["AvgGradePoints"] / s_len, 2)
    ...
    for i in range(1, s_len+1):
        ...
        newScore[i]["CreditHours"] = [credit for credit in allScore[i-1]["semester"]["credit"]]
        ...
        newScore[i]["GradePoints"] = [rating for rating in allScore[i-1]["semester"]["rating"]]
